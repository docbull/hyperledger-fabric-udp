...
func (g *Node) sendEndpoint(hostIP string) {
	...
	fmt.Println()
	fmt.Println(time.Now(), "Send Peer Endpoint to SDN Client")
	fmt.Println("Endpoint:", g.disc.Self().Endpoint)
	fmt.Println()

	_, err = conn.Write([]byte(g.disc.Self().Endpoint))
	if err != nil {
		log.Println(err)
		return
	}
...
func (g *Node) sendRemoteAddr(hostIP string) {
	...
	fmt.Println("Send remote address of virtual IP")
}
...
func (g *Node) handleMessage(m protoext.ReceivedMessage) {
	...
	if protoext.IsChannelRestricted(msg.GossipMessage) {
		if gc := g.chanState.lookupChannelForMsg(m); gc == nil {
			...
			} else if protoext.IsDataMsg(m.GetGossipMessage().GossipMessage) {
				fmt.Println("***********Recved from**********")
				fmt.Println(time.Now())
				fmt.Println(m.GetConnectionInfo().Endpoint)
				fmt.Println("********************************")
			}
			gc.HandleMessage(m)
		}
...
func (m *MemberInformation) saveMember(peer []discovery.NetworkMember) []discovery.NetworkMember {
	for i := 0; i < len(peer); i++ {
		checker := true
		for j := 0; j < len(m.Member); j++ {
			if peer[i].Endpoint == m.Member[j].Endpoint {
				checker = false
				break
			} else {
				checker = true
		...
		if checker {
			m.Member = append(m.Member, peer[i])
		}
	}
...
func (m *MemberInformation) getMember() []discovery.NetworkMember {
	return m.Member
}
...
func (g *Node) gossipBatch(msgs []*emittedGossipMessage) {
	...
	go g.myMem.saveMember(membership)
...
func (g *Node) gossipInChan(messages []*emittedGossipMessage, chanRoutingFactory channelRoutingFilterFactory) {
	...
	for len(totalChannels) > 0 {
		...
		if protoext.IsLeadershipMsg(messagesOfChannel[0].GossipMessage) {
			...
			peerName = g.selfNetworkMember().Endpoint
