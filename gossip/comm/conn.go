...
func (cs *connectionStore) getConnection(peer *RemotePeer) (*connection, error) {
	...
	if conn.peerEndpoint != "peer0.org1.example.com:7051" {
...
// D2DConnection stores D2D connection information
type D2DConnection struct {
	conn net.Listener
	err  error
}
...
type connection struct {
	...
	isD2D        bool
	errChan      *chan error
	msgChan      *chan *protoext.SignedGossipMessage
}
...
func (conn *connection) HandleD2DBlock(sock net.Conn) {
	...
	fmt.Println(conn.peerEndpoint)
	fmt.Println("size of msg:", length)
	fmt.Println()
	...
	if err != nil {
		log.Println(err)
		sock.Write([]byte("error occurs"))
		return
	}
	sock.Write([]byte("received"))

	conn.metrics.ReceivedMessages.Add(1)
	msg, err := protoext.EnvelopeToGossipMessage(protoEnvelope)
	if err != nil {
		*conn.errChan <- err
...
func (conn *connection) serviceConnection() error {
	...
	// Update errChan and msgChan for the block
